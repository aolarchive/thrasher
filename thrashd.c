...
block_addr(client_conn_t * conn, uint32_t addr) {
    ...
    /* add our hard timeou, if the option isenabled */
...
load_config(const char * file) {
    ...
    for (i = 0; c_f_in[i].parent != NULL; i++) {
        ...
        if (!g_key_file_has_key(config_file, c_f_in[i].parent, c_f_in[i].key, &error)) {
            continue;
        }
        ...
        switch (c_f_in[i].type) {
            case _c_f_t_str:
                ...
                *svar                  = g_key_file_get_string(config_file, c_f_in[i].parent, c_f_in[i].key, NULL);
                break;
            case _c_f_t_trie:
            ...
            case _c_f_t_int:
                ...
                *ivar                  = g_key_file_get_integer(config_file, c_f_in[i].parent, c_f_in[i].key, NULL);
                break;
            case _c_f_t_ratio:
                ratio                  = (block_ratio_t *)c_f_in[i].var;
                str                    = g_key_file_get_string(config_file, c_f_in[i].parent, c_f_in[i].key, NULL);
